...
const sendSelectionToBackend = () => {
  ...
  const selectedHead = document.querySelector(".head-select select").value;
  const selectedBit = document.querySelector(".bit-select select").value;

  console.log('กดกดกด')
  // Create an object with the user selections
  const requestData = {
    type_head: selectedHead,
    type_bit: selectedBit,
  };
...
// Add a click event handler for the "Export to 3D model" button
const exportButton = document.querySelector(".exportingTo");
